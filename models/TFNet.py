# Credits : https://github.com/AndreyGuzhov/TFNet-for-Environmental-Sound-Classification
import math
import torch
...
class TFBlock(nn.Module):
    def __init__(self, in_channels, out_channels, input_size):
        ...
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.alpha = nn.Parameter(torch.cuda.FloatTensor([0.1, 0.1, 0.1]))
        ...
        self.conv3 = nn.Conv2d(
            in_channels=out_channels, out_channels=1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False
        )
        self.conv4 = nn.Conv2d(
            in_channels=out_channels, out_channels=1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False
        )
        if out_channels == 64:
            self.pooling_size2 = [input_size[0], 1]
            self.pooling_size3 = [1, input_size[1]]
            self.globalAvgPool2 = nn.AvgPool2d(self.pooling_size2, stride=1)
            self.globalAvgPool3 = nn.AvgPool2d(self.pooling_size3, stride=1)
            ...
            self.pooling_size2 = [int(input_size[0] / 2), 1]
            self.pooling_size3 = [1, int(input_size[1] / 2)]
            self.globalAvgPool2 = nn.AvgPool2d(self.pooling_size2, stride=1)
            self.globalAvgPool3 = nn.AvgPool2d(self.pooling_size3, stride=1)
            ...
            self.pooling_size2 = [int(input_size[0] / 4), 1]
            self.pooling_size3 = [1, int(input_size[1] / 4)]
            self.globalAvgPool2 = nn.AvgPool2d(self.pooling_size2, stride=1)
            self.globalAvgPool3 = nn.AvgPool2d(self.pooling_size3, stride=1)
            self.globalMaxPool2 = nn.MaxPool2d((1, 128), stride=1)
            self.globalMaxPool3 = nn.MaxPool2d((128, 1), stride=1)
            ...
            self.pooling_size2 = [int(input_size[0] / 8), 1]
            self.pooling_size3 = [1, int(input_size[1] / 8)]
            self.globalAvgPool2 = nn.AvgPool2d(self.pooling_size2, stride=1)
            self.globalAvgPool3 = nn.AvgPool2d(self.pooling_size3, stride=1)
            self.globalMaxPool2 = nn.MaxPool2d((1, 128), stride=1)
            self.globalMaxPool3 = nn.MaxPool2d((128, 1), stride=1)
...
class TFNet(nn.Module):
    def __init__(self, input_size, classes_num=10, activation="softmax"):
        ...
        self.tfblock1 = TFBlock(in_channels=1, out_channels=64, input_size=input_size)
        self.tfblock2 = TFBlock(in_channels=64, out_channels=128, input_size=input_size)
        self.tfblock3 = TFBlock(in_channels=128, out_channels=256, input_size=input_size)
        self.tfblock4 = TFBlock(in_channels=256, out_channels=512, input_size=input_size)
...
class Cnn(nn.Module):
    def __init__(self, classes_num=50, activation="softmax"):
