...
class Pooling_Head(nn.Module):
    ...
    def forward(self, frame_prob, x):
        if self.pooling == "max":
            ...
            global_prob = torch.sigmoid(self.hopfield(frame_prob))
            return global_prob, frame_prob
...
class TALNetV2_meta(nn.Module):
    ...
    def __init__(
        ...
        # HEAD
        ###
        self.self_attention_meta_talnet = MultiHead(
            self.n_head, self.embedding_size, self.meta_emb, self.meta_emb, self.dropout_transfo,
        )
        ...
        self.pooling_head = Pooling_Head(
            self.embedding_size + self.meta_emb * self.num_meta, self.output_size, self.pooling,
        )
...
class TALNetV3(nn.Module):
    ...
    def forward(self, x, meta):
        ...
        if self.dropout > 0:
            meta = F.dropout(meta, p=self.dropout, training=self.training)
        meta = self.t2v(meta)
        ...
        if self.pooling == "max":
            ...
            frame_att = F.softmax(self.fc_att(x), dim=1)
            global_prob = (frame_prob * frame_att).sum(dim=1)
            return global_prob, frame_prob, frame_att, fcprob
        elif self.pooling == "auto":
            global_prob = self.autopool(frame_prob)
            return global_prob, frame_prob
...
class TALNetHOP(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention_1 = HopfieldCore(
            self.embedding_size,
            self.n_head,
            # kdim=self.d_k,
            # vdim=self.d_v,
            dropout=self.dropout_transfo,
        )
        self.fc_prob = Normed_Linear(self.embedding_size, self.output_size)
        self.pooling_head = Pooling_Head(self.embedding_size, self.output_size, self.pooling)
    ...
    def forward(self, x):
        ...
        if self.dropout > 0:
            x = F.dropout(x, p=self.dropout, training=self.training)
        x, _, _, _ = self.self_attention_1(x, x, x)  # x becomes (batch, time, embedding_size)
        if self.dropout > 0:
            x = F.dropout(x, p=self.dropout, training=self.training)
        frame_prob = torch.sigmoid(self.fc_prob(x))  # shape of frame_prob: (batch, time, output_size)
        return self.pooling_head(frame_prob, x)
...
class TALNetDense(nn.Module):
    ...
    def __init__(
        ...
        super(TALNetDense, self).__init__()
        self.n_conv_layers = n_conv_layers
        self.kernel_size = kernel_size
        self.n_pool_layers = n_pool_layers
        self.embedding_size = embedding_size
