...
class HopfieldCore(Module):
    ...
    def _reset_parameters(self):
        ...
        if self._qkv_same_embed_dim and (self.in_proj_weight is not None):
            ...
            if self.q_proj_weight is not None:
                nn.init.normal_(
                    self.q_proj_weight, mean=0.0, std=(2.0 / (self.embed_dim + self.kdim)) ** (1 / 2)
                )  # std 0.02
            if self.k_proj_weight is not None:
                nn.init.normal_(self.k_proj_weight, mean=0.0, std=(2.0 / (self.embed_dim + self.kdim)) ** (1 / 2))
            if self.v_proj_weight is not None:
                nn.init.normal_(self.v_proj_weight, mean=0.0, std=(2.0 / (self.embed_dim + self.vdim)) ** (1 / 2))
    ...
    def forward(
        ...
        assert (not self.value_as_static) or (
