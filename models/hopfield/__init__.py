    ...
    def _associate(
        ...
        if self.norm_pattern_projection is not None:
            pattern_projection = self.norm_pattern_projection(
                input=pattern_projection.reshape(shape=(-1, pattern_projection.shape[2]))
...
class StatePattern(Module):
    ...
    def __init__(self, size: int, quantity: int = 1, batch_first: bool = True, trainable: bool = True):
        """
    ...
    def forward(self, input: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        if self.__batch_first:
            state_pattern_expanded = self.state_pattern.expand(
                size=(self.state_pattern.shape[0], input.shape[0], self.state_pattern.shape[2])
            ).transpose(0, 1)
        else:
            state_pattern_expanded = self.state_pattern.expand(
