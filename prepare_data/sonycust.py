...
def remove_duplicates(dataframe, idlabel_presence_dict, consensus_threshold=0.01):
    ...
    annotation_filtered.loc[:, all_idlabel_presence] = (
        annotation_filtered[all_idlabel_presence] > consensus_threshold
    ).astype(float)
...
def clean_annotation_and_use_relabel(
    dataframe, idlabel_presence_dict, idlabel_dict, relabel_dir, consensus_threshold=0.01,
):
    """ Removes duplicate and aggregate labels
      Returns:
         annotation_filtered (pd.DataFrame): Annotations cleaned followind DCASE baseline strategie
   """
    ...
    cond3 = (
        (dataframe["split"] == "validate")
        & (dataframe["annotator_id"] != 0)
        & ~dataframe["audio_filename"].isin(valid_exclusion)
    )
    ...
    relabeled_df[idlabel_dict["coarse"] + idlabel_dict["fine"]] = (
        relabeled_df[idlabel_dict["coarse"] + idlabel_dict["fine"]] > 0.7
    ).astype(float)
    rename_column = dict(
        zip(
            idlabel_dict["coarse"] + idlabel_dict["fine"],
            idlabel_presence_dict["coarse"] + idlabel_presence_dict["fine"],
        )
    )
...
class SONYCUST(Dataset):
    ...
    resources = [
        ...
        (
            "https://zenodo.org/record/3873076/files/audio-eval-0.tar.gz",
            "audio-eval-0",
            "f92136a4a77c626a0900c18217c908bf",
        ),
        (
            "https://zenodo.org/record/3873076/files/audio-eval-1.tar.gz",
            "audio-eval-1",
            "b9a727afcdb794572bf3d60b879e30a8",
        ),
        (
            "https://zenodo.org/record/3873076/files/audio-eval-2.tar.gz",
            "audio-eval-2",
            "05b06fd8e274e6b8ae9202bd09fddc56",
        ),
        (
            "https://zenodo.org/record/3873076/files/annotations.csv",
            "annotations.csv",
            "5292e1eed610775a4460103989db2d69",
        ),
        (
            "https://zenodo.org/record/3873076/files/dcase-ust-taxonomy.yaml",
            "dcase-ust-taxonomy.yaml",
            "6c1cca1c4c383a6ebb0cb71cb74fe3a9",
        ),
    ]
    all_metadata_labels = [
        "sensor_id",
    ...
    def __init__(self, sonycust_folder, mode, metadata=None, transform=None, download=False):
        ...
        self.file_path_dict = {name: os.path.join(self.sonycust_folder, name) for _, name, _ in self.resources}
        ...
        self.annotation_df = cleaning_annotation_baseline(self.raw_annotation_df, self.idlabel_presence_dict)
    ...
    def download(self):
        ...
        for url, filename, md5 in self.resources[0:4]:
            download_and_extract_archive(
                url, download_root=self.sonycust_folder, filename=filename + ".tar.gz", md5=md5, remove_finished=True,
            )
        ...
        for eval_num in range(3):
            for f in os.listdir(self.file_path_dict["audio-eval-" + str(eval_num)]):
                if f.endswith(".wav"):
                    shutil.move(
                        os.path.join(self.file_path_dict["audio-eval-" + str(eval_num)], f),
                        self.file_path_dict["audio"],
                    )
    ...
    def compute_melspec(self):
        ...
        audio_list_path = [
            os.path.join(self.file_path_dict["audio"], x)
        ...
        def compute_one_mel(filename):
            ...
            np.save(
                os.path.join(self.file_path_dict["melTALNet"], os.path.basename(filename)[:-3] + "npy",), logmel,
            )
    ...
    def parse_taxonomy(self):
        ...
        full_fine_idlabel_list = [
            "{}-{}_{}".format(coarse_id, fine_id, fine_label)
            for coarse_id, fine_dict in taxonomy["fine"].items()
            for fine_id, fine_label in fine_dict.items()
        ]
        fine_idlabel_list = [x for x in full_fine_idlabel_list if x.split("_")[0].split("-")[1] != "X"]
        coarse_idlabel_list = ["_".join([str(k), v]) for k, v in taxonomy["coarse"].items()]

        idlabel_dict = {
            "full_fine": full_fine_idlabel_list,
        ...
        idlabel_presence_dict = {
            "full_fine": full_fine_idlabel_presence_list,
    ...
    def __getitem__(self, index):
        ...
        if sr != 48000:
            # about 3 or 4 audios are concerned
        ...
        if self.mode == "both":
            proximity = {
                "full_fine": np.array(self.annotation_df[self.idlabel_proximity_dict["full_fine"]].iloc[index]),
            ...
            label = {
                "full_fine": np.array(self.annotation_df[self.idlabel_presence_dict["full_fine"]].iloc[index]),
...
class SONYCUST_TALNet(SONYCUST):
    def __init__(
        ...
        super().__init__(
            sonycust_folder, mode, metadata=metadata, transform=transform, download=download,
        )
        ...
        if cleaning_strat == "DCASE":
            self.annotation_df = cleaning_annotation_baseline(
                self.raw_annotation_df, self.idlabel_presence_dict, consensus_threshold=consensus_threshold,
            )
        elif cleaning_strat == "Relabeled":
            self.annotation_df = clean_annotation_and_use_relabel(
                self.raw_annotation_df,
                self.idlabel_presence_dict,
                self.idlabel_dict,
                self.file_path_dict["relabeled_df"],
            )
        elif cleaning_strat == "All_unique":
            self.annotation_df = remove_duplicates(
                self.raw_annotation_df, self.idlabel_presence_dict, consensus_threshold=consensus_threshold,
            )
      ...
      """
        ...
        if self.one_hot_time and self.to_one_hot != []:
            for features in self.to_one_hot:
                one_hot_enc = np.array(
                    one_hot(self.annotation_df[features].iloc[index] - 1, self.one_hot_dict[features],)
                )
